...
On a aussi tenté d'implémenter une version des Watched Literals avec un seul litéral surveillé, mais sans aller jusqu'au bout du débuggage par manque de temps.
...
Des tests préalables nous ont montré que SmartWatchedClause n'était souvent pas meilleure que WatchedClause. Nous ne l'avons donc pas fait apparaître sur les courbes, mais il nous sera facile de l'ajouter plus tard.
On n'a pas non plus fait apparaître RAND et DLIS avec tri initial, mais le tri est innutile dans ces deux cas. Le tri initial n'était là que pour comparer l'implémentation de BASIC avec et sans tri initial.

D'une part, on observe qu'il y a un pic au niveau du temps d'exécution autour d'un ratio nbClause/nbVar de 4-5. Ce pic est très marqué pour l'heuristique RAND.

Globalement, nos temps d'exécutions sont plus irréguliers que ceux de minisat, mais en moyenne assez comparables (même si cette tendance a l'air de se dissiper lorsque le nombre de variables augmente, dès 80 par exemple).

On remarque aussi que WatchedDLIS donne des performances très satisfaisantes, le plus souvent meilleures que les autres heuristiques/implémentations, bien qu'elle ai été codée naivement.

La différence entre SmartClause/ConstAssignClause et BasicClause n'était pas très marquée car on est resté sur des problèmes de type 3-sat, mais lorsque la taille des clauses augmentera, la différence se fera très probablement plus sentir.
